...
myForm.addEventListener("submit", (e) =>{
   ...
   //Enrolled Students Language
   const e_language = document.createElement("p");
   e_language.id="e_language";
   e_std_data.appendChild(e_language);
   var e_checkbox = document.getElementsByName("language");
   for (i=0 ; i<e_checkbox.length ; i++){
       if(e_checkbox[i].checked){
           i===0 ? 
            language.innerText += e_checkbox[i].value:
            language.innerText += ", " + e_checkbox[i].value;
       }
   }
   e_std_data.appendChild(e_language);

   //Enrolled Students Skills
